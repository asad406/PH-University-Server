...
const getAllCourseFromDB = async (query: Record<string, unknown>) => {
    const courseQuery = new QueryBuilder(Course.find().populate('preRequisiteCourse.course'), query).search(courseSearchableFields).filter()
        .sort()
        .paginate()
        .fields()
...
const updateCourseIntoDB = async (id: string, payload: Partial<TCourse>) => {
    ...
    try {
        ...
        if (!updatedBasicCourseInfo) {
            throw new AppError(httpStatus.BAD_REQUEST, 'Failed to delete preRequisite course')
        }
        ...
        if (preRequisiteCourse && preRequisiteCourse?.length > 0) {
            ...
            if (!newPreRequisiteCourse) {
                throw new AppError(httpStatus.BAD_REQUEST, 'Failed to updated preRequisite course')
            }
        }
